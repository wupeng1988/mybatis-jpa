...
public class XMLMapperBuilder extends BaseBuilder {
    ...
    private void embeddedEntityResultMap(Class entityClass) {
        ResultMap resultMap = EntitySqlDispatcher.getInstance().getMetaDataParser(entityClass).getResultMap();
        ResultMapResolver resultMapResolver = new ResultMapResolver(builderAssistant, resultMap.getId(), entityClass, null, null, resultMap.getResultMappings(), null);
        try {
    ...
    private void embeddedEntitySql(Class entityClass) {
        ...
        for (Table.SqlSegment sql : table.getSqlSegments()) {
            String id = sql.getId();
            id = builderAssistant.applyCurrentNamespace(id, false);
            configuration.getStaticSqlFragments().put(id, sql.getSql());
        }
    }
